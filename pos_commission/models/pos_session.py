...
class PosSession(models.Model):
    ...
    def action_pos_session_closing_control(self):
        ...
        if employee:
            active_commissions_boss = self.env['commission.type'].search([('active','=',True),('boss_ids','in',employee.id)])
            if active_commissions_boss:
                pos_orders_boss = self.env['pos.order'].search([('session_id','=', self.id), ('state','=','paid')])
                for pos_order_boss in pos_orders_boss:
                    for active_commission_boss in active_commissions_boss:
                        commission_line = {
                            'bosses': employee.id,
                            'sellers': False,
                            'commission': active_commission_boss.id,
                            'order': pos_order_boss.id,
                            'company': ','.join([c.name for c in active_commission_boss.company_id]),
                            'store': ','.join([s.name for s in active_commission_boss.store_id]),
                            'point_of_sale': ','.join([pos.name for pos in active_commission_boss.point_of_sale_id]),
                        }
                        if active_commission_boss.commission_type == 'standard':
                            commission_line['commission_type'] = 'standard'
                            commission_line['tree_product'] = 'All Products'
                            if active_commission_boss.standard_commission_type == 'percentage':
                                commission_line['amount'] = pos_order_boss.amount_paid * active_commission_boss.percentage_commission / 100
                                self.env['commission.line'].create(commission_line)
                            else:
                                commission_line['amount'] = active_commission_boss.fixed_commission
                                self.env['commission.line'].create(commission_line)
                        else:
                            commission_line['commission_type'] = 'product_categ'
                            for commission_lines in active_commission_boss.commission_line_ids:
                                if commission_lines.applied_on == '4_global':
                                    if commission_lines.compute_price == 'percentage':
                                        commission_line['amount'] = pos_order_boss.amount_paid * commission_lines.percent_price / 100
                                        self.env['commission.line'].create(commission_line)
                                    else:
                                        commission_line['amount'] = commission_lines.fixed_price * len(pos_order_boss.lines.ids)
                                        self.env['commission.line'].create(commission_line)
                                if commission_lines.applied_on == '3_season':
                                    ...
                                    amount = 0
                                    for line in pos_order_boss.lines:
