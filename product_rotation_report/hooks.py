...
def pre_init_hook(cr):
    ...
    logger.info("Create product_rotation_report function for data report")

    query = query_create_function
    cr.execute(query)

def uninstall_hook(cr, registry):
    logger = logging.getLogger(__name__)
    logger.info("Drop product_rotation_report function for data report")

    query = """
    drop view if exists product_rotation_report;
    drop function if exists product_rotation_report(date,date);
        """
    cr.execute(query)
