    ...
    show: function (options) {
        ...
        if (options) {
			if (options.auto_close) {
				...
				self.close();
				self.$el.find('.popup').append('<div class="footer"><button class="button accept seller_btn" id="btn-accept">Continuar</button><button class="button cancel seller_btn">Cerrar</button></div>');
			}
			if (options.module_pos_seller == true){
				options.sellers.forEach(function (seller){
					self.$el.find(".seller-list").append("<li><label><input type='radio' class='seller_name' name='sellers' value='" + seller['id'] + "'><span>"+ seller['name']+ "</span></label></li>");
				})
				if(self.pos.seller != false){
